<template>
  <PageHeader title="Cập nhật sản phẩm" :backTo="{ name: 'products.list' }" />

  <div v-if="loaded" class="createLayout">
...
onMounted(async () => {
  rawProduct.value = await productService.get(route.params.id);
  patchFormFromProduct(rawProduct.value);
  loaded.value = true;
});

function valueByLabel(options, label) {
  return options.find((x) => String(x.label) === String(label))?.value || "";
}

function patchFormFromProduct(product) {
  const variants =
  ...
  form.value = {
    ...form.value,
    maSanPham: product.maSanPham || product.sku || "",
    tenSanPham: product.tenSanPham || product.name || "",
    giaBan: Number(product.giaBan ?? product.price) || 0,
    giaNhap: Number(product.giaNhap) || 0,
    idDanhMuc:
      product.idDanhMuc ||
      valueByLabel(
        categoryOptions,
        product.tenDanhMuc || product.categoryName || product.category,
      ),
    idThuongHieu:
      product.idThuongHieu ||
      valueByLabel(
        brandOptions,
        product.tenThuongHieu || product.brandName || product.brand,
      ),
    idXuatXu:
      product.idXuatXu || valueByLabel(originOptions, product.tenXuatXu || ""),
    idMucDichChay:
      product.idMucDichChay ||
      valueByLabel(purposeOptions, product.tenMucDichChay || ""),
    idCoGiay:
      product.idCoGiay ||
      valueByLabel(shoePartOptions, product.tenCoGiay || ""),
    idChatLieu:
      product.idChatLieu ||
      valueByLabel(materialOptions, product.tenChatLieu || ""),
    idDeGiay:
      product.idDeGiay ||
      valueByLabel(soleCostOptions, product.tenDeGiay || ""),
    gioiTinhKhachHang: product.gioiTinhKhachHang || "",
    hinhAnh: product.hinhAnh || "",
    moTaNgan: product.moTaNgan || "",
    moTaChiTiet: product.moTaChiTiet || "",
    trangThai: product.trangThai || product.status || "ACTIVE",
    variants,
...
async function handleSubmit() {
  ...
  try {
    ...
    await productService.update(route.params.id, productPayload);
