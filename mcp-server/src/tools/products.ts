    ...
    handler: async ({ query }: { query: string }) => {
        try {
            ...
            if (products.length === 0) {
                return {
                    content: [{ type: "text" as const, text: `Không tìm thấy sản phẩm nào khớp với từ khóa "${query}".` }]
                };
            }
            ...
            return {
                content: [{ type: "text" as const, text: "Lỗi khi gọi API tìm kiếm: " + (error as Error).message }]
            };
        }
    }
};
