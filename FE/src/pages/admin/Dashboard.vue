...
<template>
  <div class="flex-1 flex flex-col space-y-6 min-h-0">
    ...
    <AdminDataFilter @search="(q) => (searchQuery = q)">
      <template #filters>
        <select
          ...
          class="border border-gray-300 rounded-lg text-sm px-3 py-2 bg-white outline-none focus:ring-1 focus:ring-aurora focus:border-aurora"
        >
          <option>All</option>
    ...
    <AdminDataTable :columns="columns" :data="filteredProducts" :current-page="1" :total-pages="1">
      <template #cell-name="{ row }">
        <div class="flex items-center gap-3">
          <div
            class="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center font-bold text-xs border border-gray-100 italic"
          >
            {{ row.name[0] }}
          </div>
