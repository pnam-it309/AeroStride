        ...
        <template v-if="props.active || props.warm">
            <div class="digital-bg">
                <div v-if="currentShoe.bgType === 'rain'" class="bg-scene scene-rain">
                    ...
                    <div v-for="particle in rainParticles" :key="particle.id" class="particle-rain"
                ...
                <div v-else-if="currentShoe.bgType === 'cubes'" class="bg-scene scene-cubes">
                    <div v-for="particle in cubeParticles" :key="particle.id" class="floating-cube"
                ...
                <div v-else class="bg-scene scene-orbits">
                    <div v-for="ring in orbitRings" :key="ring.id" class="ripple-ring"
                        :style="{ animationDelay: ring.delay, borderColor: currentShoe.color }"></div>
                </div>
                ...
                <v-row no-gutters class="fill-height align-center">
                    <v-col cols="12" class="position-relative">
                        <div class="shoe-focus-container">
                            <div class="shoe-item">
                                <div class="shoe-meta-fixed reveal-container" :class="{ active: props.active }">
                                    ...
                                    <v-btn variant="outlined" :color="currentShoe.color"
                                        class="mt-4 px-8 font-weight-black explore-btn reveal-item delay-3" height="48"
                                        rounded="pill" :to="currentShoe.id ? `/product/${currentShoe.id}` : '/shoes'">
                                        KHÁM PHÁ CHI TIẾT
                                ...
                                <div class="shoe-visual-wrapper-center">
                                    <div class="micro-details">
                                        <div v-for="(feat, idx) in currentShoe.features" :key="idx" class="feature-node"
                                            :style="{
                                            ...
                                            <div class="node-content"
                                                :style="{ textAlign: activeCard === 1 ? 'left' : 'right' }">
                                                ...
                                                <div class="node-hud"
                                                    :style="{ color: currentShoe.color, fontWeight: '800' }">
                                                    <span v-if="idx === 0">ĐỘ BỀN VẬT LIỆU // {{ hudValues[0] }}%</span>
                                                    <span v-else-if="idx === 1">HẤP THỤ XUNG LỰC // {{ hudValues[1]
                                                        }}%</span>
                                    ...
                                    <model-viewer :src="currentShoe.modelUrl || '/models/Shoe.glb'"
                                        :poster="currentShoe.image || '/assets/images/products/1.jpg'"
                                        :alt="currentShoe.title" camera-controls auto-rotate rotation-per-second="30deg"
                                        shadow-intensity="1" environment-image="neutral" class="the-shoe object-contain"
                                        :style="{ transform: currentShoe.rotation, width: '100%', maxWidth: '500px', height: '400px', zIndex: 10, position: 'relative', outline: 'none' }"></model-viewer>
                                    <div class="shadow-floor" :style="{
                                        background: `radial-gradient(ellipse at center, ${currentShoe.color}26 0%, transparent 70%)`
                                    }"></div>
                                    <div class="shoe-aura" :style="{ boxShadow: `0 0 160px ${currentShoe.color}1a` }">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="controls-overlay">
                            <v-btn icon size="x-large" variant="plain" color="blue-darken-4" class="nav-btn"
                                @click="prevCard">
                                <v-icon size="48">mdi-chevron-left</v-icon>
                            </v-btn>

                            <div class="step-indicator">
                                <span v-for="n in 3" :key="n" class="step-line"
                                    :class="{ active: activeCard === n - 1 }" @click="activeCard = n - 1"></span>
                            </div>
