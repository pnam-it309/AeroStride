...
const timelineSteps = computed(() => {
    if (order.value?.loaiDon === 'TAI_QUAY' || order.value?.loaiDon === 'OFFLINE') {
        return [ORDER_STATUS.CHO_XAC_NHAN, ORDER_STATUS.HOAN_THANH];
    }
    return [ORDER_STATUS.CHO_XAC_NHAN, ORDER_STATUS.XAC_NHAN, ORDER_STATUS.CHO_GIAO, ORDER_STATUS.DANG_GIAO, ORDER_STATUS.HOAN_THANH];
});

// Đơn ở trạng thái bất thường -> không hiển thị thanh ngang mà báo riêng
const isCancelled = computed(() => order.value?.trangThai === ORDER_STATUS.DA_HUY || order.value?.trangThai === ORDER_STATUS.HOAN_DON);
...
<template>
    <div class="order-detail-page min-vh-100 pb-12 bg-white">
        ...
        <v-container class="py-8" style="max-width: 1400px">
            ...
            <v-btn
                variant="text"
                class="font-weight-bold text-none mb-6 back-btn"
                style="color: #1e257c"
                ...
                <div class="order-header mb-8 pb-6" style="border-bottom: 2px solid #1e257c">
                    <div class="d-flex align-center justify-space-between flex-wrap ga-4">
                        ...
                        <div class="text-right d-flex flex-column align-end ga-2">
                            <v-chip style="background: #1e257c; color: white" variant="flat" size="large" class="font-weight-bold px-4">
                    ...
                    <v-col cols="12" lg="4" md="5" class="d-flex flex-column ga-6">
                        ...
                        <div class="section-block pa-6">
                            ...
                            <div class="shipping-details">
                                ...
                                <div v-if="order.ngayDuKienNhan" class="shipping-row">
                                    ...
                                    <p class="text-body-2 font-weight-bold mb-0" style="color: #1e257c">
                                        {{ formatDateFull(order.ngayDuKienNhan) }}
                                    </p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="payment-details pa-4" style="background: #f8f9fa">
                                ...
                                <div class="d-flex justify-space-between mb-3">
                                    ...
                                    <span
                                        ...
                                        :class="!order.phiVanChuyen || order.phiVanChuyen == 0 ? 'text-blue-darken-4' : ''"
                                    >
                                        <v-icon v-if="!order.phiVanChuyen || order.phiVanChuyen == 0" size="14" color="#1e257c" class="mr-1"
                                ...
                                <div v-if="order.tienGiam > 0" class="d-flex justify-space-between mb-3">
                                    ...
                                    <span class="text-body-2 font-weight-medium" style="color: #1e257c"
                                        >-{{ formatPrice(order.tienGiam) }}</span
                                    >
                                </div>
                                <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 12px 0" />
                                <div class="d-flex justify-space-between align-center mb-2">
                                    <span class="text-body-1 font-weight-bold">Tổng cộng</span>
                                    <span class="text-h6 font-weight-bold" style="color: #1e257c">{{
                        ...
                        <div v-if="order.choPhepThanhToanLai" class="section-block pa-6 text-center">
                            ...
                            <p class="text-caption mb-4">
                                Đơn hàng chuyển khoản chưa được thanh toán. Bạn có thể thanh toán lại qua VNPay.
                            </p>
                            <v-btn
                                style="background: #1e257c; color: white"
                                variant="flat"
                        ...
                        <div v-if="canCancel" class="section-block pa-6 text-center">
                            ...
                            <v-btn
                                style="color: #1e257c; border-color: #1e257c"
                                variant="outlined"
                                rounded="pill"
                                class="font-weight-bold text-none"
                                block
        ...
        <ReviewModal v-model:show="showReviewModal" :order="order" @review-success="handleReviewSuccess" />
    </div>
</template>
