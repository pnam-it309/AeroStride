...
const addrFormTitle = computed(() => {
    if (showAddrForm.value) {
        return isEditAddr.value ? 'Cập nhật địa chỉ' : 'Thêm địa chỉ mới';
    }
    return 'Thêm địa chỉ khác';
});
...
const saveBtnText = computed(() => {
    return isEditAddr.value ? 'Cập nhật địa chỉ' : 'Lưu địa chỉ';
});
...
<template>
    <v-container fluid class="pa-4 animate-fade-in font-body"
        ...
        <AdminTable title="Danh sách khách hàng" addButtonText="Tạo mới" show-export-button
        ...
        <v-dialog v-model="addrDialog" max-width="1000" width="80vw" scrollable transition="dialog-bottom-transition">
            <v-card class="rounded-xl font-body addr-dialog-card d-flex flex-column">
                <!-- Tiêu đề dialog -->
                ...
                <v-card-text class="pa-0 d-flex flex-column addr-dialog-body" style="min-height: 500px; max-height: 80vh; overflow: hidden">
                    <v-row no-gutters class="flex-grow-1 addr-dialog-grid">
                        <!-- ── Cột trái: Danh sách địa chỉ ── -->
                        <v-col cols="12" md="6" class="px-6 pt-2 pb-4 bg-white addr-scroll-pane addr-scroll-left">
                            <div
                                class="d-flex align-center justify-space-between px-5 pt-4 pb-2 sticky-sub-header bg-white border-b">
                            ...
                            <div v-else-if="listDiaChi.length === 0"
                                class="text-center py-16 px-4 bg-slate-50/30 rounded-lg mx-5 mt-4">
                                <v-icon style="font-size: 36px !important; color: #1e293b !important;"
                                    class="mb-4">mdi-map-marker-off</v-icon>
                                ...
                                <div v-for="addr in listDiaChi" :key="addr.id"
                                    ...
                                    <div class="d-flex align-center gap-2 mt-5">
                                        <v-btn v-if="!addr.laMacDinh" variant="tonal" size="small" color="success"
                                            class="text-none rounded-lg px-4" height="32" style="font-weight: 400"
                        ...
                        <v-col cols="12" md="6" class="px-6 pt-2 pb-4 bg-white addr-scroll-pane addr-scroll-right">
                            <div
                                class="px-5 pt-4 pb-2 d-flex align-center justify-space-between sticky-sub-header bg-white border-b">
                            ...
                            <div v-if="!showAddrForm"
                                class="text-center py-16 px-4 bg-slate-50/30 rounded-lg mx-5 mt-4 border border-dashed border-slate-200">
                                <v-icon style="font-size: 36px !important; color: #1e293b !important;"
                                    class="mb-4">mdi-map-marker-plus</v-icon>
                                <div class="text-slate-500"
                                    style="font-size: 14px !important; font-weight: 400 !important;">
                                ...
                                <v-row dense>
                                    ...
                                    <v-col cols="12">
                                        ...
                                        <v-autocomplete v-model="addrForm.tinh" :items="provinces" item-title="name"
                                            item-value="code" placeholder="Chọn tỉnh" variant="outlined" hide-details
                                    ...
                                    <v-col cols="12" md="6">
                                        ...
                                        <v-autocomplete v-model="addrForm.thanhPho" :items="districts" item-title="name"
                                            ...
                                            hide-details :disabled="!addrForm.tinh" :loading="loadingLoc.districts" />
                                    </v-col>
                                    <v-col cols="12" md="6">
                                        ...
                                        <v-autocomplete v-model="addrForm.phuongXa" :items="wards" item-title="name"
                                            ...
                                            hide-details :disabled="!addrForm.thanhPho" :loading="loadingLoc.wards" />
                                    </v-col>
                                    <v-col cols="12">
                                    ...
                                    <v-col cols="12" class="mt-2">
                                        <v-btn color="primary" variant="flat" block
                                            class="text-none font-weight-bold rounded-lg" height="48"
                                            :loading="addrSaving" :disabled="!addrForm.tenNguoiNhan ||
                                                ...
                                                " @click="saveAddress">
                                            <span class="force-white-text">{{ saveBtnText }}</span>
                                        </v-btn>
                                    </v-col>
                                </v-row>
                            </v-form>
