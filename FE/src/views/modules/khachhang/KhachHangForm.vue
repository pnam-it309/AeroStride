...
const fetchProvinces = async () => {
  ...
  } catch (e) { console.error(e); } finally { loadingLocations.value.provinces = false; }
};
...
const fetchDistricts = async (provinceCode) => {
  ...
  } catch (e) { console.error(e); } finally { loadingLocations.value.districts = false; }
};
...
const fetchWards = async (districtCode) => {
  ...
  } catch (e) { console.error(e); } finally { loadingLocations.value.wards = false; }
};
  ...
  confirmDialog.value = {
    ...
    action: async () => {
      ...
      try {
        ...
        addNotification({ title: 'Lỗi', subtitle: error.response?.data?.message || 'Có lỗi xảy ra khi lưu thông tin', color: 'error' });
      } finally {
        saving.value = false;
...
const openAddrDialog = (item = null) => {
    if (item) {
        ...
        addrForm.value = { tinh: null, thanhPho: null, phuongXa: null, diaChiChiTiet: '', tenNguoiNhan: customerForm.value.ten, sdtNguoiNhan: customerForm.value.sdt, laMacDinh: listDiaChi.value.length === 0 };
    }
...
const handleDeleteAddr = (id) => {
    confirmDialog.value = {
        show: true, title: 'Xóa địa chỉ', message: 'Bạn có chắc muốn xóa địa chỉ này?', color: 'error',
        action: async () => {
            try {
  ...
  <v-container fluid class="pa-6 gray-bg min-h-screen font-body">
    ...
    <div class="d-flex align-center mb-6">
      ...
      <div>
        ...
        <p class="text-subtitle-1 text-medium-emphasis">
          {{ isDetailView ? 'Chi tiết lịch sử và thông tin liên hệ khách hàng' : 'Hệ thống quản lý thông tin khách hàng & địa điểm giao hàng' }}
        </p>
      </div>
      <v-spacer></v-spacer>
      ...
      <v-col cols="12" lg="8">
        ...
        <v-card class="rounded-xl border shadow-none mb-6 pb-2">
           ...
           <v-card-text class="pa-0">
              ...
              <div v-else v-for="(addr, idx) in listDiaChi.ref" :key="addr.id" class="pa-6 d-flex align-start border-b hover-bg-light transition-all">
                 ...
                 <div class="flex-grow-1">
                    <div class="d-flex align-center gap-2 mb-1">
                       ...
                       <v-chip v-if="addr.laMacDinh" color="success" size="x-small" variant="flat" class="font-weight-bold">MẶC ĐỊNH</v-chip>
                    </div>
           ...
           <v-card-text class="pa-6">
              <v-row>
                 ...
                 <v-col cols="12" md="6">
                    ...
                    <div class="text-subtitle-1 text-medium-emphasis italic font-italic font-weight-medium">Dữ liệu được mã hóa bảo mật</div>
                 </v-col>
              </v-row>
           </v-card-text>
        </v-card>
      ...
      <v-col cols="12" lg="8">
        ...
        <v-card class="rounded-xl border shadow-sm mb-6" elevation="0">
          ...
          <v-card-text class="pa-6">
            <v-row>
              <v-col cols="12" md="4">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Mã khách hàng <span class="text-caption text-primary">(Hệ thống tự tạo)</span></div>
                <v-text-field v-model="customerForm.ma" readonly placeholder="KH-XXXX" variant="outlined" density="comfortable" class="font-weight-black" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="8">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Họ và tên *</div>
                <v-text-field v-model="customerForm.ten" :readonly="isDetailView" placeholder="Ví dụ: Nguyễn Văn A" variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Email *</div>
                <v-text-field v-model="customerForm.email" :readonly="isDetailView" placeholder="khachhang@gmail.com" variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Số điện thoại *</div>
                <v-text-field v-model="customerForm.sdt" :readonly="isDetailView" placeholder="09xx.xxx.xxx" variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Ngày sinh</div>
                <v-text-field v-model="customerForm.ngaySinh" :readonly="isDetailView" type="date" variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Giới tính</div>
                <v-select v-model="customerForm.gioiTinh" :readonly="isDetailView" :items="[{title: 'Nam', value: true}, {title: 'Nữ', value: false}]" variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-select>
              </v-col>
        ...
        <v-card class="rounded-xl border shadow-sm mb-6" elevation="0">
          <v-card-title class="pa-6 border-b d-flex align-center">
            ...
            <v-spacer></v-spacer>
            <v-btn v-if="isEditMode" color="primary" variant="tonal" size="small" class="text-none" prepend-icon="mdi-plus" @click="openAddrDialog()">Thêm địa chỉ</v-btn>
          </v-card-title>
          <v-card-text class="pa-6">
            ...
            <div v-else v-for="addr in listDiaChi" :key="addr.id" class="mb-4 pa-4 border rounded-xl d-flex align-center gap-4 hover-addr">
              ...
              <div class="flex-grow-1">
                <div class="d-flex align-center gap-2 mb-1">
                  ...
                  <v-chip size="x-small" color="primary" variant="tonal" class="ml-2 px-2 font-weight-bold">{{ addr.maDiaChi }}</v-chip>
                  <v-chip v-if="addr.laMacDinh" color="success" size="x-small" variant="flat" class="ml-1">Mặc định</v-chip>
                </div>
                <div class="text-subtitle-2 text-medium-emphasis">{{ addr.diaChiChiTiet }}, {{ addr.phuongXa }}, {{ addr.thanhPho }}, {{ addr.tinh }}</div>
              </div>
              <div class="d-flex align-center gap-1" v-if="!isDetailView">
                <v-btn icon variant="text" size="small" color="primary" @click="openAddrDialog(addr)"><v-icon>mdi-pencil</v-icon></v-btn>
                <v-btn v-if="!addr.laMacDinh" icon variant="text" size="small" color="error" @click="handleDeleteAddr(addr.id)"><v-icon>mdi-delete</v-icon></v-btn>
                <v-btn v-if="!addr.laMacDinh" variant="tonal" size="small" color="info" class="text-none ml-2" @click="handleSetDefault(addr.id)">Đặt mặc định</v-btn>
              </div>
            </div>
          </v-card-text>
        </v-card>
        ...
        <v-card class="rounded-xl border shadow-sm" elevation="0">
          ...
          <v-card-text class="pa-6">
            ...
            <v-textarea v-model="customerForm.ghiChu" :readonly="isDetailView" placeholder="Ghi chú về khách hàng..." variant="outlined" rows="2" class="font-weight-medium" hide-details></v-textarea>
          </v-card-text>
        </v-card>
      </v-col>
      ...
      <v-col cols="12" lg="4">
        <v-card class="rounded-xl border shadow-sm mb-6" elevation="0">
          ...
          <v-card-text class="pa-6 text-center">
            ...
            <v-text-field v-if="!isDetailView" v-model="customerForm.hinhAnh" placeholder="https://..." variant="outlined" density="compact" hide-details class="font-weight-medium"></v-text-field>
          </v-card-text>
        </v-card>
        ...
        <v-card class="rounded-xl border shadow-sm" elevation="0">
          ...
          <v-card-text class="pa-6">
            ...
            <v-text-field v-model="customerForm.tenTaiKhoan" :readonly="true" placeholder="Username" variant="outlined" density="comfortable" class="font-weight-medium mb-4" hide-details></v-text-field>
            <div v-if="!isDetailView" class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Mật khẩu</div>
            <v-text-field v-if="!isDetailView" v-model="customerForm.matKhau" type="password" variant="outlined" density="comfortable" class="font-weight-medium" placeholder="••••••••" hide-details></v-text-field>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
    ...
    <v-dialog v-model="addrDialog" max-width="600">
      <v-card class="rounded-xl border shadow-2xl">
        <v-card-title class="pa-4 font-weight-bold border-b text-primary">{{ isEditAddr ? 'Cập nhật địa chỉ' : 'Thêm địa chỉ mới' }}</v-card-title>
        <v-card-text class="pa-6">
          <v-row>
            <v-col cols="12" md="6">
              <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Tên người nhận *</div>
              <v-text-field v-model="addrForm.tenNguoiNhan" placeholder="Nhập tên" variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-text-field>
            </v-col>
            <v-col cols="12" md="6">
              <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Số điện thoại *</div>
              <v-text-field v-model="addrForm.sdtNguoiNhan" placeholder="09xx..." variant="outlined" density="comfortable" class="font-weight-medium" hide-details></v-text-field>
            </v-col>
            <v-col cols="12" md="4">
              <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Tỉnh / Thành phố *</div>
              <v-autocomplete v-model="addrForm.tinh" :items="provinces" item-title="name" item-value="code" placeholder="Chọn" variant="outlined" density="comfortable" hide-details :loading="loadingLocations.provinces" @update:model-value="(val) => { addrForm.thanhPho = null; addrForm.phuongXa = null; if(val) fetchDistricts(val); }"></v-autocomplete>
            </v-col>
            <v-col cols="12" md="4">
              <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Quận / Huyện *</div>
              <v-autocomplete v-model="addrForm.thanhPho" :items="districts" item-title="name" item-value="code" placeholder="Chọn" variant="outlined" density="comfortable" hide-details :loading="loadingLocations.districts" :disabled="!addrForm.tinh" @update:model-value="(val) => { addrForm.phuongXa = null; if(val) fetchWards(val); }"></v-autocomplete>
            </v-col>
            <v-col cols="12" md="4">
              <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Phường / Xã *</div>
              <v-autocomplete v-model="addrForm.phuongXa" :items="wards" item-title="name" item-value="code" placeholder="Chọn" variant="outlined" density="comfortable" hide-details :loading="loadingLocations.wards" :disabled="!addrForm.thanhPho"></v-autocomplete>
            </v-col>
            <v-col cols="12">
              <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Địa chỉ cụ thể *</div>
              <v-textarea v-model="addrForm.diaChiChiTiet" placeholder="Số nhà, đường..." variant="outlined" rows="2" class="font-weight-medium" hide-details></v-textarea>
            </v-col>
            <v-col cols="12" v-if="listDiaChi.length > 0">
              <v-checkbox v-model="addrForm.laMacDinh" label="Đặt làm địa chỉ mặc định" color="primary" hide-details density="compact"></v-checkbox>
            </v-col>
          </v-row>
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions class="pa-4 bg-grey-lighten-4">
          ...
          <v-btn color="primary" variant="flat" class="px-6 text-none font-weight-medium rounded-lg" @click="saveAddress" :disabled="!addrForm.tenNguoiNhan || !addrForm.sdtNguoiNhan || !addrForm.tinh || !addrForm.thanhPho || !addrForm.phuongXa || !addrForm.diaChiChiTiet">Lưu địa chỉ</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
.hover-addr { transition: all 0.2s; }
.hover-addr:hover { background-color: #f1f5f9; border-color: #cbd5e1 !important; transform: translateY(-1px); }
</style>
