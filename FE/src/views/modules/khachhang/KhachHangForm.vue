...
const sortedDiaChiList = computed(() => {
    const list = listDiaChi.value.length > 0 ? listDiaChi.value : (customerForm.value.listDiaChi || []);
    // Sắp xếp: Mặc định luôn được ưu tiên hàng đầu
    return [...list].sort((a, b) => {
        if (a.laMacDinh && !b.laMacDinh) return -1;
        if (!a.laMacDinh && b.laMacDinh) return 1;
        return 0;
    });
});
...
const filteredPurchaseHistory = computed(() => {
    ...
    if (invoiceFilters.value.search) {
        const s = invoiceFilters.value.search.toLowerCase();
        data = data.filter(inv => 
            (inv.maHoaDon || inv.ma || '').toLowerCase().includes(s)
        );
    }
    return data;
});
...
const paginatedPurchaseHistory = computed(() => {
    const start = (invoicePagination.value.page - 1) * invoicePagination.value.size;
    const end = start + invoicePagination.value.size;
    return filteredPurchaseHistory.value.slice(start, end);
});
...
const viewInvoiceDetail = async (invoice) => {
    ...
    try {
        ...
        // Map items with fallbacks for all common field names to avoid N/A displays
        const items = (fullDetail?.listsHoaDonChiTiet || fullDetail?.chiTiets || fullDetail || []).map(item => ({
            ...item,
            maSanPham: item.maSanPham || item.maSP || item.sanPhamMa || item.chiTietSanPham?.sanPham?.ma || 'N/A',
            tenSanPham: item.tenSanPham || item.sanPhamTen || item.chiTietSanPham?.sanPham?.ten || 'Sản phẩm không tên',
            maBienThe: item.maChiTietSanPham || item.maCTSP || item.maBienThe || item.maBienTheSanPham || item.chiTietSanPham?.ma || 'N/A',
            tenMauSac: item.tenMauSac || item.mauSacTen || item.chiTietSanPham?.mauSac?.ten || 'N/A',
            tenKichThuoc: item.tenKichThuoc || item.kichThuocTen || item.chiTietSanPham?.kichThuoc?.ten || 'N/A',
            soLuong: item.soLuong || 0,
            donGia: item.donGia || item.giaTien || item.chiTietSanPham?.giaBan || 0,
            thanhTien: item.thanhTien || ((item.soLuong || 0) * (item.donGia || item.giaTien || item.chiTietSanPham?.giaBan || 0))
        }));
...
const getOrderTypeClass = (orderType) => {
    ...
    return type.includes('ONLINE') || type.includes('GIAO_HANG') ? 'order-type-online' : 'order-type-offline';
};

const getOrderTypeLabel = (orderType) => {
    const type = String(orderType || '').toUpperCase();
    return type.includes('ONLINE') || type.includes('GIAO_HANG') ? 'GIAO HÀNG' : 'TẠI QUẦY';
};
...
const onFileChange = async (e) => {
    ...
    customerForm.value.hinhAnh = localUrl;
    ...
    <v-container id="khach-hang-form-container" fluid class="pa-6 animate-fade-in overflow-y-auto font-body"
        ...
        <v-row v-else-if="isDetailView">
            <v-col cols="12" lg="4">
                ...
                <v-card class="filter-card elevation-0 mb-6 text-center h-100">
                    <v-card-text class="pa-5">
                        <v-avatar size="140" color="blue-lighten-5" class="mb-4 border-xl border-white elevation-4">
                            <v-img :key="customerForm.hinhAnh" :src="customerForm.hinhAnh || FB_DEFAULT_AVATAR"></v-img>
                        </v-avatar>
                        ...
                        <v-chip size="small" variant="flat" :class="[
                            'status-chip',
                            customerForm.trangThai === 'DANG_HOAT_DONG' ? 'status-chip-active' : 'status-chip-inactive'
                        ]" class="px-8 mb-8">
                        ...
                        <div class="text-left">
                            <v-row dense>
                                <v-col cols="6">
                                    <div class="d-flex align-center mb-3 bg-slate-50 pa-3 rounded-xl border h-100">
                                        ...
                                        <div class="flex-grow-1 overflow-hidden">
                                            ...
                                            <div class="text-subtitle-2 text-slate-700 text-truncate">{{ customerForm.sdt }}</div>
                                        </div>
                                    </div>
                                </v-col>
                                <v-col cols="6">
                                    <div class="d-flex align-center mb-3 bg-slate-50 pa-3 rounded-xl border h-100">
                                        <div class="icon-blob bg-info-lighten-4 mr-3">
                                            <CalendarIcon color="info" size="18" />
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden">
                                            <div class="text-caption font-weight-medium text-slate-400">Ngày sinh</div>
                                            <div class="text-subtitle-2 text-slate-700 text-truncate">
                                                {{ customerForm.ngaySinh || 'Chưa cập nhật' }}
                                            </div>
                                        </div>
                                    </div>
                                </v-col>
                                <v-col cols="6">
                                    <div class="d-flex align-center bg-slate-50 pa-3 rounded-xl border h-100">
                                        <div class="icon-blob bg-purple-lighten-5 mr-3">
                                            <HashIcon color="purple" size="18" />
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden">
                                            <div class="text-caption font-weight-medium text-slate-400">Mã khách hàng</div>
                                            <div class="text-subtitle-2 text-slate-700 text-truncate">{{ customerForm.ma }}</div>
                                        </div>
                                    </div>
                                </v-col>
                                <v-col cols="6">
                                    <div class="d-flex align-center bg-slate-50 pa-3 rounded-xl border h-100">
                                        <div class="icon-blob bg-amber-lighten-5 mr-3">
                                            <GenderTransgenderIcon color="amber" size="18" />
                                        </div>
                                        <div class="flex-grow-1 overflow-hidden">
                                            <div class="text-caption font-weight-medium text-slate-400">Giới tính</div>
                                            <div class="text-subtitle-2 text-slate-700 text-truncate">
                                                {{ customerForm.gioiTinh === true ? 'Nam' : customerForm.gioiTinh === false ? 'Nữ' : 'Khác' }}
                                            </div>
                                        </div>
                                    </div>
                                </v-col>
            ...
            <v-col cols="12" lg="8">
                ...
                <v-card class="filter-card elevation-0 mb-6 pb-0 h-100 d-flex flex-column">
                    <v-card-text class="pa-5 flex-grow-1 d-flex flex-column">
                        <div class="section-header d-flex align-center mb-4">
                            ...
                            <span class="text-subtitle-1 font-weight-black text-slate-800">Danh sách địa chỉ nhận
                                hàng</span>
                            <v-chip color="primary" size="small" variant="tonal" class="ml-3 font-weight-black">
                                {{ listDiaChi.length > 0 ? listDiaChi.length : customerForm.listDiaChi?.length || 0 }}
                                Địa chỉ
                            </v-chip>
                        </div>
                        ...
                        <AdminEmptyState
                            v-if="listDiaChi.length === 0 && (!customerForm.listDiaChi || customerForm.listDiaChi.length === 0)"
                            icon="mdi-map-marker-off" title="Chưa có địa chỉ nhận hàng"
                            subtitle="Khách hàng chưa đăng ký địa chỉ nhận hàng nào trong hệ thống." />

                        <!-- Hiển thị toàn bộ địa chỉ có cuộn - Phóng to để chiếm trọn không gian thẻ -->
                        <div v-else class="address-scroll-container pr-2 flex-grow-1" style="overflow-y: auto; height: 0;">
                            <div v-for="(addr, idx) in sortedDiaChiList"
                                :key="addr.id || idx"
                                ...
                                <div class="flex-grow-1">
                                    <div class="d-flex align-center gap-2 mb-1">
                                        ...
                                        <v-chip v-if="addr.laMacDinh" color="success" size="x-small" variant="flat"
                                            class="font-weight-medium px-3">MẶC ĐỊNH</v-chip>
                                    </div>
        ...
        <v-row v-if="isDetailView && !loading" class="mb-6 mt-n2">
            <!-- Row 2: System Notes and Account Security side-by-side -->
            <v-col cols="12" md="6">
                <v-card class="filter-card elevation-0 h-100">
                    <v-card-text class="pa-8">
                        ...
                        <div class="text-body-2 font-weight-medium text-slate-500 leading-relaxed min-h-24">
                ...
                <v-card class="filter-card elevation-0 h-100">
                    <v-card-text class="pa-8">
                        <div class="section-header d-flex align-center mb-6">
                            ...
                            <span class="text-subtitle-1 font-weight-bold text-slate-800">Thông tin tài khoản</span>
                        </div>
                        <v-row>
                            <v-col cols="12" sm="6">
                                <div class="bg-slate-50 pa-4 rounded-xl border h-100">
                                    <div class="text-caption font-weight-medium text-slate-400 mb-1">Tên đăng nhập</div>
                                    <div class="text-subtitle-1 font-weight-medium text-primary text-truncate">
                                        {{ customerForm.tenTaiKhoan || 'Chưa thiết lập' }}
                                    </div>
                                </div>
                            </v-col>
                            <v-col cols="12" sm="6">
                                <div class="bg-slate-50 pa-4 rounded-xl border d-flex align-center h-100">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <div class="text-caption font-weight-medium text-slate-400 mb-1">Mật khẩu</div>
                                        <div class="text-subtitle-2 text-slate-400 italic font-italic text-truncate">Dữ liệu được mã
                                            hóa bảo mật</div>
                                    </div>
        ...
        <!-- Purchase History Dashboard Full Width -->
        <v-row v-if="isDetailView">
            <v-col cols="12">
                <div class="mb-6"></div>
                <div class="invoice-filter-shell mb-4">
                    <AdminFilter title="Bộ lọc lịch sử" :loading="invoiceLoading" hide-refresh @search="invoicePagination.page = 1">
                        <v-col cols="12" md="4">
                            <div class="filter-field-label">Tìm theo mã hóa đơn</div>
                            <v-text-field
                ...
                <AdminTable 
                    ...
                    :headers="[
                        { text: 'STT', width: '60px' },
                        { text: 'Mã hóa đơn', width: '130px' },
                        { text: 'Ngày mua', width: '140px' },
                        { text: 'Loại đơn', width: '120px' },
                        { text: 'Số lượng', width: '100px' },
                        { text: 'Tổng tiền', width: '150px' },
                        { text: 'Trạng thái', width: '150px' },
                        { text: 'Hành động', width: '90px' }
                    ]"
                    :items="paginatedPurchaseHistory"
                    :total-count="filteredPurchaseHistory.length"
                ...
                >
                    <template #row="{ item, index }">
                        <tr class="data-row">
                            <td class="data-cell text-center text-slate-400">
                                {{ (invoicePagination.page - 1) * invoicePagination.size + index + 1 }}
                            </td>
                            <td class="data-cell text-left font-weight-bold text-slate-700">
                            ...
                            <td class="data-cell text-center text-slate-600 font-weight-bold">
                                {{ item.tongSanPham || item.soLuong || item.tongSoLuong || 0 }}
                            </td>
                            ...
                            <td class="data-cell text-center">
                                <v-chip v-if="getOrderStatusMeta(item.trangThai)"
                                    :class="['font-weight-black status-chip', getOrderStatusMeta(item.trangThai).chipClass]"
                                    variant="flat" size="x-small">
                            ...
                            <td class="data-cell text-center">
                                <v-btn icon variant="text" size="small" color="primary" class="action-btn"
                    ...
                    <template #pagination>
                        <AdminPagination
                            v-model="invoicePagination.page"
                            :page-size="invoicePagination.size"
                            @update:pageSize="invoicePagination.size = $event"
                            :total-pages="Math.ceil(filteredPurchaseHistory.length / invoicePagination.size) || 1"
                            :total-elements="filteredPurchaseHistory.length"
                            :current-size="paginatedPurchaseHistory.length"
                            @change="() => {}"
                        />
                    </template>
                </AdminTable>
            ...
            <v-col cols="12" lg="8">
                ...
                <v-card class="filter-card elevation-0 mb-6">
                    <v-card-text class="pa-8">
                        ...
                        <AdminEmptyState v-else-if="listDiaChi.length === 0" icon="mdi-map-marker-off"
                            title="Chưa có địa chỉ"
                            subtitle="Bạn có thể thêm địa chỉ mới cho khách hàng này để thuận tiện cho việc giao hàng." />
                    ...
                    <v-card-text class="pa-8 text-center">
                        ...
                        <div class="position-relative d-inline-block mx-auto mb-6">
                            <v-avatar size="160" color="blue-lighten-5"
                                ...
                                <v-img :key="customerForm.hinhAnh" :src="customerForm.hinhAnh || FB_DEFAULT_AVATAR"
                                    cover>
            ...
            <v-card class="premium-card rounded-xl khach-hang-dialog-card">
                ...
                <v-card-text class="pa-6">
                    ...
                    <div v-else>
                            ...
                            <table class="native-admin-table w-100">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="header-cell text-center">Mã SP</th>
                                        <th class="header-cell text-center">Tên SP</th>
                                        <th class="header-cell text-center">Mã BT</th>
                                        <th class="header-cell text-center">Màu</th>
                                        <th class="header-cell text-center">Size</th>
                                        <th class="header-cell text-center" style="width: 80px">SL</th>
                                        <th class="header-cell text-center" style="width: 120px">Đơn giá</th>
                                        <th class="header-cell text-center" style="width: 130px">Thành tiền</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(item, index) in selectedInvoice?.items" :key="item.id" class="data-row">
                                        ...
                                        <td class="data-cell text-center">
                                            <span class="mono-font text-slate-500">{{ item.maSanPham }}</span>
                                        </td>
                                        <td class="data-cell text-left">
                                            <span class="text-slate-800 font-weight-medium">{{ item.tenSanPham }}</span>
                                        </td>
                                        <td class="data-cell text-center">
                                            <v-chip size="x-small" variant="tonal" color="slate-500" class="mono-font">
                                                {{ item.maBienThe }}
                                            </v-chip>
                                        </td>
                                        <td class="data-cell text-center">
                                            <v-chip size="x-small" variant="flat" color="slate-100" class="text-slate-600">
                                                {{ item.tenMauSac }}
                                            </v-chip>
                                        </td>
                                        <td class="data-cell text-center">
                                        ...
                                        <td class="data-cell text-center">
                                            <span class="text-slate-600 font-weight-bold">{{ item.soLuong }}</span>
                                        </td>
                                        <td class="data-cell text-right text-slate-600 px-4">
                                            {{ (item.donGia || 0).toLocaleString() }}đ
                                        </td>
                                        <td class="data-cell text-right text-primary font-weight-bold px-4"
                        ...
                        <!-- Tổng tiền bên ngoài table -->
                        <div class="mt-6 d-flex justify-end align-center px-4">
                            <span class="text-subtitle-1 font-weight-black text-slate-700 mr-4">Tổng tiền:</span>
                            <span class="text-h6 font-weight-black text-primary" style="color: #1e257c !important">
...
<style scoped>
/* Scoped styles for KhachHangForm dashboard */
.italic {
    font-style: italic;
}
...
/* Giới hạn hiển thị cho lịch sử mua hàng để tránh khoảng trắng dư thừa */
.history-table-scroll {
    height: auto !important;
    max-height: 600px; /* Đủ cho toolbar + header + 5-6 dòng + pagination */
...
.history-table-scroll :deep(.table-wrapper) {
    max-height: 400px;
    overflow-y: auto;
    flex: 0 0 auto !important; /* Tuyệt đối không cho phép chiếm thêm không gian */
}

.history-table-scroll :deep(thead th) {
    position: sticky;
    top: 0;
    z-index: 10;
...
/* 
   ...
   Sử dụng ID cụ thể để đảm bảo độ ưu tiên cao nhất
*/
