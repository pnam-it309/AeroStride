...
<template>
    <v-container fluid class="pa-4 animate-fade-in font-body"
                        ...
                        <div class="d-flex align-center bg-white pa-4 rounded-lg ">
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex justify-space-between mb-2">
                                    <span class="text-caption font-weight-black text-slate-600">Lọc theo giá bán sản
                                        phẩm</span>
                                    <span class="text-caption font-weight-black text-primary">
                                        {{ formatCurrency(filters.khoangGia[0]) }} - {{
                                        formatCurrency(filters.khoangGia[1]) }}
                                    </span>
                                </div>
        ...
        <AdminTable title="Danh sách sản phẩm" addButtonText="Tạo mới" :headers="[
            ...
            <template #actions>
                <template v-if="selectedProductIds.length > 0">
                    <v-btn prepend-icon="mdi-check-circle-outline" variant="flat" class="bulk-btn-success text-none" :disabled="!canBulkActivateProducts"
                        @click="handleBulkProductStatus('DANG_HOAT_DONG')">
                        Bật trạng thái
                    </v-btn>
                    <v-btn prepend-icon="mdi-close-circle-outline" variant="flat" class="bulk-btn-warning text-none" :disabled="!canBulkDeactivateProducts"
            ...
            <template #top>
                <div class="px-6 py-3 bg-slate-50 border-b d-flex align-center flex-wrap gap-3">
                ...
                <tr class="data-row">
                    <td class="data-cell text-center">
                        <v-checkbox-btn :model-value="selectedProductIds.includes(item.id)" color="primary" hide-details
                            density="compact" @update:model-value="toggleProductSelection(item.id, $event)" />
                    </td>
