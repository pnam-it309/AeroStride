...
import { XIcon, DeviceFloppyIcon, RefreshIcon, PhotoIcon, PlusIcon, TrashIcon } from 'vue-tabler-icons';
import { dichVuFile } from '@/services/core/dichVuFile';
import { useNotifications } from '@/services/notificationService';
import { dichVuMauSac, dichVuKichThuoc } from '@/services/product/dichVuThuocTinh';
import AttributeQuickAddModal from './AttributeQuickAddModal.vue';
...
const handleQuickAddSaved = (newEntity) => {
    formData.value[quickAdd.value.targetField] = newEntity.id;
...
const autoCreateAttribute = async (val, field, service, type, label) => {
    try {
        ...
        if (type === 'MAU_SAC') payload.maMauHex = '#000000'; // Default black for quick add
        ...
        // Wait a bit for props to update or just set it
        setTimeout(() => {
...
const generateSKU = () => {
    ...
    const color = props.options.mauSacs.find(m => m.id === formData.value.idMauSac);
    const size = props.options.kichThuocs.find(k => k.id === formData.value.idKichThuoc);
...
watch(() => [formData.value.idMauSac, formData.value.idKichThuoc], generateSKU);
...
const handleImageUpload = async (event) => {
    ...
    try {
        const url = await dichVuFile.taiLenFile(file);
...
<template>
    <v-dialog v-model="props.open" max-width="700" persistent transition="dialog-bottom-transition" scrollable>
        <v-card class="rounded-xl border shadow-2xl p-0">
            ...
            <v-card-title class="px-8 py-6 border-b d-flex align-center justify-space-between bg-slate-50/50">
                <div>
                    ...
                    <p class="text-subtitle-2 text-slate-500 font-weight-medium mt-1">Thiết lập SKU, tồn kho và giá cho
                        sản phẩm.</p>
                </div>
            </v-card-title>
                        ...
                        <div class="space-y-5">
                            <div class="form-group">
                                <div class="d-flex align-center justify-space-between mb-2 px-1">
                                    <span
                                        class="text-caption font-weight-bold text-slate-700 text-uppercase tracking-wider">Màu
                                        sắc</span>
                                ...
                                <v-combobox v-model="formData.idMauSac" :items="options.mauSacs" item-title="ten"
                                    item-value="id" :disabled="mode === 'edit'" placeholder="Chọn màu sắc"
                                    variant="outlined" density="comfortable" hide-details :return-object="false"
                                    class="modern-select"
                            ...
                            <div class="form-group">
                                <div class="mb-2 px-1">
                                    <span
                                        class="text-caption font-weight-bold text-slate-700 text-uppercase tracking-wider">Kích
                                        thước</span>
                                </div>
                                <v-combobox v-model="formData.idKichThuoc" :items="options.kichThuocs" item-title="ten"
                                    item-value="id" :disabled="mode === 'edit'" placeholder="Chọn kích thước"
                                    variant="outlined" density="comfortable" hide-details :return-object="false"
                                    class="modern-select"
                            ...
                            <div class="form-group">
                                <div class="mb-2 px-1">
                                    <span
                                        class="text-caption font-weight-bold text-slate-700 text-uppercase tracking-wider">Mã
                                        SKU</span>
                                </div>
                                <v-text-field v-model="formData.maChiTietSanPham" placeholder="Hệ thống tự động tạo..."
                            ...
                            <div class="form-group">
                                <div class="mb-2 px-1">
                                    <span
                                        class="text-caption font-weight-bold text-slate-700 text-uppercase tracking-wider">Hình
                                        ảnh</span>
                                </div>
                                <div
                                    class="variant-image-upload border-dashed rounded-xl pa-2 d-flex align-center gap-4 bg-slate-50/50">
                                    <v-avatar rounded="lg" size="80" class="border bg-white shadow-sm">
                                        <v-img v-if="formData.urlAnh" :src="formData.urlAnh" cover></v-img>
                                        <div v-else
                                            class="fill-height d-flex align-center justify-center text-slate-300">
                                            <PhotoIcon size="32" />
                                        </div>
                                    </v-avatar>
                                    <div class="flex-grow-1">
                                        ...
                                        <p class="text-caption text-slate-400 mt-2 line-height-tight">Dung lượng tối đa
                                            2MB. Định dạng JPG, PNG.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <v-col cols="12" md="6">
                        <div class="space-y-5">
                            <div class="form-group">
                                <div class="mb-2 px-1">
                                    <span
                                        class="text-caption font-weight-bold text-slate-700 text-uppercase tracking-wider">Số
                                        lượng tồn</span>
                                </div>
                            ...
                            <div class="form-group">
                                <div class="mb-2 px-1">
                                    <span
                                        class="text-caption font-weight-bold text-slate-700 text-uppercase tracking-wider">Giá
                                        nhập (VNĐ)</span>
                                </div>
                                <v-text-field v-model.number="formData.giaNhap" type="number" min="0" variant="outlined"
                                    density="comfortable" hide-details placeholder="0" suffix="₫"
                            ...
                            <div class="form-group">
                                ...
                                <v-text-field v-model.number="formData.giaBan" type="number" min="0" variant="outlined"
                                    density="comfortable" hide-details placeholder="0" suffix="₫"
                                    class="modern-input"></v-text-field>
                            </div>
                        </div>
            ...
            <v-card-actions class="px-8 py-6 bg-slate-50 d-flex justify-end gap-3">
                ...
                <v-btn color="primary" variant="flat" class="px-8 rounded-xl font-weight-black h-11 text-white" :loading="submitting" @click="handleSubmit">
                    <template #prepend>
