  ...
  <v-container fluid class="pa-4 animate-fade-in font-body"
    ...
    <div class="flex-grow-1 min-h-0">
      <AdminTable title="Danh mục biến thể" :headers="[
        ...
        { text: 'Mã sản phẩm chi tiết', width: '150px' },
        ...
        :showExportButton="true" :exportButtonText="variantExportButtonText" @add="openCreateVariantModal"
        @export="handleExportVariants" class="h-100">

        <template #actions>
          <div class="d-flex align-center gap-2">
            <v-btn v-if="selectedVariantIds.length > 0" prepend-icon="mdi-qrcode" variant="flat" class="admin-btn-secondary text-none"
              @click="handleExportVariantQrExcel">
              {{ variantQrExportButtonText }}
            </v-btn>
            <v-btn v-if="selectedVariantIds.length > 0" prepend-icon="mdi-check-circle" variant="flat" class="bulk-btn-success text-none"
              :disabled="!canBulkActivateVariants" @click="handleBulkVariantStatus('DANG_HOAT_DONG')">
              Bật trạng thái
            </v-btn>
            <v-btn v-if="selectedVariantIds.length > 0" prepend-icon="mdi-minus-circle" variant="flat" class="bulk-btn-warning text-none"
              :disabled="!canBulkDeactivateVariants" @click="handleBulkVariantStatus('KHONG_HOAT_DONG')">
              Tắt trạng thái
            </v-btn>
          </div>
        </template>
