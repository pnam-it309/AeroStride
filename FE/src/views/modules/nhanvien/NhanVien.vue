...
<template>
    <v-container fluid class="pa-4 animate-fade-in font-body"
        ...
        <AdminTable :title="tab === 0 ? 'Danh sách nhân viên' : 'Yêu cầu tạo mới mật khẩu'"
            :addButtonText="tab === 0 ? 'Tạo mới' : 'Tạo mới'" :hide-add-button="tab === 1" show-export-button
            :headers="tableHeaders" :items="tab === 0 ? employees : pendingRequests"
            :total-count="pagination.totalElements" :loading="loading" @add="router.push(PATH.NHAN_VIEN_FORM)"
            @export="handleExport">
            <template #top>
                <v-tabs v-model="tab" bg-color="transparent" color="primary" height="54" align-tabs="start"
                    class="admin-tabs">
                    <v-tab :value="0" class="text-none font-weight-bold px-4 tab-item">
                        <v-icon start size="16">mdi-view-grid-outline</v-icon>
                        Danh sách nhân viên
                        <v-chip v-if="pagination.totalElements > 0" size="x-small"
                    ...
                    <v-tab :value="1" class="text-none font-weight-bold px-4 tab-item">
                        ...
                        <v-chip v-if="pendingRequests.length > 0" size="x-small"
            ...
            <template #row="{ item, index }">
                <tr class="data-row">
                    ...
                    <td class="data-cell action-cell">
                        ...
                        <v-btn v-else color="primary" size="small" variant="flat" class="text-none"
...
*/
:deep(.purchase-history-table td),
:deep(.invoice-items-table td) {
