    ...
    <div class="d-flex align-center mb-6">
      ...
      <div>
        ...
        <p class="text-subtitle-1 text-medium-emphasis">
          {{ isDetailView ? 'Theo dõi hiệu năng và danh sách sản phẩm đang được áp dụng' : 'Thiết lập chương trình ưu đãi và giảm giá theo sự kiện' }}
        </p>
      </div>
      <v-spacer></v-spacer>
    ...
    <v-row v-else-if="isDetailView">
       <v-col cols="12" lg="4">
          <v-card class="rounded-xl border shadow-none mb-6 text-center pa-6">
             ...
             <div class="mt-2 mb-4">
                <v-chip :color="form.trangThai === 'DANG_HOAT_DONG' ? 'success' : 'info'" variant="flat" class="font-weight-bold px-6">
                ...
                <div class="d-flex flex-column gap-6">
                   <div class="d-flex align-center">
                      ...
                      <div>
                         ...
                         <div class="text-subtitle-1 font-weight-black text-dark">{{ new Date(form.ngayBatDau).toLocaleString('vi-VN') }}</div>
                      </div>
                   </div>
                   <div class="d-flex align-center">
                      ...
                      <div>
                         ...
                         <div class="text-subtitle-1 font-weight-black text-dark">{{ new Date(form.ngayKetThuc).toLocaleString('vi-VN') }}</div>
                      </div>
                   </div>
                </div>
             </v-card-text>
       ...
       <v-col cols="12" lg="8">
             ...
             <v-card-text class="pa-6">
                ...
                <div class="d-flex flex-wrap gap-8 py-2">
                   <div>
                      ...
                      <div class="text-h6 font-weight-black text-primary">{{ new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(form.dieuKienGiamGia) }}</div>
                   </div>
                   <div>
          ...
          <v-card class="rounded-xl border shadow-none">
             <v-card-title class="pa-5 border-b d-flex align-center justify-space-between">
                ...
                <v-chip color="info" size="small" variant="tonal" class="font-weight-bold">{{ selectedVariants.length }} sản phẩm</v-chip>
             </v-card-title>
                ...
                <v-table class="product-apply-table">
                   ...
                   <tbody>
                      <tr v-for="vId in selectedVariants" :key="vId">
                         <td class="py-4 px-6">
                            <div class="font-weight-bold text-dark">{{ products.find(p => p.id === vId)?.tenSanPhamDayDu || 'Sản phẩm ' + vId }}</div>
                         </td>
                         ...
                         <td class="text-right py-4 px-6 font-weight-black text-error">
                            {{ new Intl.NumberFormat('vi-VN').format((products.find(p => p.id === vId)?.giaBan || 0) * (form.loaiGiamGia === 'PHAN_TRAM' ? (1 - form.soTienGiam / 100) : 1) - (form.loaiGiamGia === 'TIEN_MAT' ? form.soTienGiam : 0)) }}
                         </td>
                      </tr>
                   </tbody>
                </v-table>
      ...
      <v-col cols="12" lg="8">
        <v-card class="rounded-xl border shadow-none mb-6">
          ...
          <v-card-text class="pa-6">
            <v-row>
              <v-col cols="12" md="6">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Mã đợt giảm giá * <span class="text-caption text-primary">(Hệ thống tự tạo)</span></div>
                <v-text-field v-model="form.ma" readonly placeholder="Hệ thống tự tạo..." variant="outlined" density="comfortable" hide-details class="mb-4 font-weight-bold"></v-text-field>
              </v-col>
              <v-col cols="12" md="6">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Tên đợt giảm giá *</div>
                <v-text-field v-model="form.ten" :readonly="isDetailView" placeholder="Ví dụ: Siêu Sale Hè 2024" variant="outlined" density="comfortable" hide-details class="mb-4"></v-text-field>
              </v-col>
              <v-col cols="12">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Mô tả</div>
                <v-textarea v-model="form.moTa" :readonly="isDetailView" variant="outlined" rows="3" placeholder="Chi tiết chương trình..." hide-details></v-textarea>
              </v-col>
            </v-row>
          </v-card-text>
        </v-card>
          ...
          <v-card-text class="pa-6">
            <v-row>
              <v-col cols="12" md="4">
                ...
                <v-select v-model="form.loaiGiamGia" :readonly="isDetailView" :items="[{title:'Phần trăm (%)',value:'PHAN_TRAM'},{title:'Số tiền mặt (VNĐ)',value:'TIEN_MAT'}]" variant="outlined" density="comfortable" hide-details></v-select>
              </v-col>
              <v-col cols="12" md="4">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">{{ form.loaiGiamGia === 'PHAN_TRAM' ? 'Giá trị giảm (%) *' : 'Số tiền giảm (₫) *' }}</div>
                <v-text-field v-model.number="form.soTienGiam" :readonly="isDetailView" variant="outlined" density="comfortable" type="number" :suffix="form.loaiGiamGia === 'PHAN_TRAM' ? '%' : '₫'" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="4">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Đơn tối thiểu (₫) *</div>
                <v-text-field v-model.number="form.dieuKienGiamGia" :readonly="isDetailView" variant="outlined" density="comfortable" type="number" suffix="₫" hide-details></v-text-field>
              </v-col>
              <v-col cols="12" md="4">
                <div class="text-subtitle-2 font-weight-bold mb-1 ml-1 text-dark">Mức độ ưu tiên</div>
                <v-text-field v-model.number="form.mucUuTien" :readonly="isDetailView" variant="outlined" density="comfortable" type="number" placeholder="Cao hơn sẽ ưu tiên hiển thị trước" hide-details></v-text-field>
              </v-col>
            </v-row>
          </v-card-text>
        </v-card>
        ...
        <v-card class="rounded-xl border shadow-none mt-6">
          ...
          <v-card-text class="pa-6">
            ...
            <v-autocomplete
              ...
              :hint="selectedVariants.length > 0 ? `Đã chọn ${selectedVariants.length} sản phẩm` : 'Trống bản chất sẽ không áp dụng cho sản phẩm nào'"
              persistent-hint
            >
              <template #item="{ props, item }">
                <v-list-item v-bind="props" :subtitle="`Giá: ${item.raw.giaBan} VNĐ | Kho: ${item.raw.soLuong}`"></v-list-item>
              </template>
            </v-autocomplete>
          </v-card-text>
        </v-card>
