 ...
 * Thành phần: VariantFormModal
 * Chức năng: Biểu mẫu thêm/sửa chi tiết một biến thể, gồm màu, size, SKU, tồn kho,
 *            giá nhập, giá bán và ảnh đại diện gửi về ProductVariantRequest.
 *            Hỗ trợ tự động tạo nhanh thuộc tính nếu nhập giá trị chưa tồn tại.
 */
...
// Tạo mẫu dữ liệu rỗng cho biểu mẫu.
...
// Trích xuất URL hình ảnh từ dữ liệu tải lên có nhiều kiểu trường khác nhau.
...
// Trích xuất ảnh đại diện chính từ một mảng ảnh.
...
// Tổng hợp tất cả trường khả thi để lấy URL ảnh của biến thể.
...
// Lấy giá trị của thuộc tính nằm sâu trong dữ liệu lồng nhau.
...
// Lọc mục trong ô chọn có tìm kiếm bằng thuật toán không phân biệt có/không dấu.
...
// Cố gắng tìm ID đúng nhất của một tùy chọn dựa theo tiêu đề hoặc chữ hiển thị.
...
// Trả về mảng tùy chọn mới, đảm bảo luôn chứa tùy chọn hiện tại để tránh có ID nhưng thiếu text.
...
// Bắt sự kiện khi người dùng nhấn Enter trong ô chọn có tìm kiếm; nếu là chữ mới thì tự tạo nhanh.
...
// Tự động gửi API để tạo nhanh thuộc tính như màu hoặc size.
...
// Hàm điền dữ liệu của biến thể hiện tại vào biểu mẫu khi mở chế độ sửa.
...
// Xử lý sự kiện sau khi chọn ảnh, tải lên Cloud và lấy URL.
