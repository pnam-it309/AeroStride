...
const fetchProvinces = async () => {
    ...
    finally { loadingLoc.value.provinces = false; }
};
...
const fetchDistricts = async (code) => {
    ...
    finally { loadingLoc.value.districts = false; }
};
...
const fetchWards = async (code) => {
    ...
    finally { loadingLoc.value.wards = false; }
};
...
const timelineSteps = computed(() => {
    ...
    const steps = [
        { key: 0, label: getStatusLabel(0), icon: CircleCheckIcon, note: "Đơn hàng đang chờ duyệt" },
        { key: 1, label: getStatusLabel(1), icon: CheckIcon, note: "Đơn hàng đã được duyệt" },
        { key: 2, label: getStatusLabel(2), icon: PackageIcon, note: "Đang chuẩn bị hàng hóa" },
        { key: 3, label: getStatusLabel(3), icon: TruckIcon, note: "Đơn hàng đang được giao" },
        { key: 6, label: getStatusLabel(6), icon: CircleCheckIcon, note: "Đơn hàng đã kết thúc thành công" }
    ];

    // Terminal branch for cancellations/returns
    if (status === 7) {
        steps.push({ key: 7, label: getStatusLabel(7), icon: CircleXIcon, note: "Đơn hàng đã bị hủy" });
    } else if (status === 8) {
        steps.push({ key: 8, label: getStatusLabel(8), icon: CircleXIcon, note: "Yêu cầu hủy từ khách hàng" });
    } else if (status === 9) {
        steps.push({ key: 9, label: getStatusLabel(9), icon: CircleXIcon, note: "Đơn hàng đã hoàn" });
    }

    const currentIndex = status === null ? -1 : steps.findIndex(s => s.key === status);
    
    // Only show steps up to the current status to keep the timeline focused
    ...
    return visibleSteps.map((step, index) => ({
        ...
        state: index === currentIndex ? "active" : "done",
...
const openEditShipping = () => {
    ...
    editInfoForm.value = {
        ...
        district: null,
        ward: null,
        detailAddress: order.value.diaChiNguoiNhan || order.value.diaChi || ""
    };
...
const saveInfoUpdate = async (type) => {
    ...
    try {
        const payload = { ...editInfoForm.value };
        if (type === 'shipping') {
            const p = provinces.value.find(x => x.code === editInfoForm.value.province)?.name || "";
            const d = districts.value.find(x => x.code === editInfoForm.value.district)?.name || "";
            const w = wards.value.find(x => x.code === editInfoForm.value.ward)?.name || "";
            if (p && d && w) {
                payload.diaChiNguoiNhan = `${editInfoForm.value.detailAddress}, ${w}, ${d}, ${p}`;
            } else {
                payload.diaChiNguoiNhan = editInfoForm.value.detailAddress;
            }
        }
        await dichVuHoaDon.capNhatThongTinHoaDon(order.value.id, payload);
...
<template>
    <v-container fluid class="pa-6 animate-fade-in screen-scroll">
            ...
            <div class="header-section">
                <div class="header-left">
                    <div class="d-flex align-center header-left-row">
                        ...
                        <v-chip v-if="loaded && getOrderStatusMeta(order.trangThai)"
                            :color="getStatusTone(order.trangThai)" variant="flat" class="font-bold px-4 status-chip text-white">
                    ...
                    <div v-if="loaded" class="text-caption text-slate-500 mt-1 d-flex align-center">
                        ...
                        <v-tooltip text="Thay đổi nhân viên tiếp nhận" location="top">
                            <template v-slot:activator="{ props }">
                                <v-btn v-bind="props" icon variant="text" size="small" color="primary" class="ml-1" @click="openEditReceptionist">
                                    <EditIcon size="16" />
                                </v-btn>
                            </template>
                        </v-tooltip>
                ...
                <div class="header-right d-flex gap-3">
                    ...
                    <v-menu v-if="order && getOrderStatus() !== null && getOrderStatus() < 3">
                        <template v-slot:activator="{ props }">
                            <v-btn color="primary" variant="flat" class="rounded-lg px-6 font-bold text-white"
                                height="44" v-bind="props" :loading="updatingStatus">
                                <EditIcon size="18" class="mr-2 text-white" />
                                Cập nhật trạng thái
                            </v-btn>
                        </template>
            ...
            <v-col cols="12" lg="8">
                ...
                <v-card class="premium-card mb-6">
                    <div class="card-title pa-5 border-b d-flex align-center justify-space-between bg-slate-50">
                        ...
                        <v-btn variant="flat" color="primary" size="small" class="rounded-md px-4 font-weight-bold text-white"
                            @click="openEditInfo">
                            <EditIcon size="16" class="mr-2 text-white" />
                            Chỉnh sửa
                        </v-btn>
                    </div>
                    <v-card-text class="pa-6">
                        <v-row class="align-center">
                            <v-col cols="12" md="3">
                                <div class="d-flex align-center">
                                    <v-avatar color="primary" size="56" class="mr-3 rounded-xl shadow-lg">
                                        <span class="text-h6 text-white font-weight-bold">{{ (order.tenKhachHang || 'K').charAt(0) }}</span>
                                    </v-avatar>
                                    <div class="overflow-hidden">
                                        <div class="text-subtitle-1 font-weight-bold text-slate-900 text-truncate">
                                            {{ order.tenKhachHang || order.khachHang?.ten || 'Khách lẻ' }}
                                        </div>
                                        <div class="text-caption text-slate-400 font-weight-bold">
                                            SĐT: {{ order.soDienThoai || order.khachHang?.sdt || order.soDienThoaiNguoiNhan || 'N/A' }}
                                        </div>
                                    </div>
                                </div>
                            </v-col>
                            <v-col cols="12" md="3">
                                <div class="pl-md-4 border-l-md">
                                    <div class="text-caption font-weight-bold text-slate-400 text-uppercase mb-1">Email</div>
                                    <div class="text-subtitle-2 font-weight-bold text-slate-700 text-truncate">
                                        {{ order.email || order.khachHang?.email || 'N/A' }}
                                    </div>
                                </div>
                            </v-col>
                            <v-col cols="12" md="3">
                                <div class="pl-md-4 border-l-md">
                                    <div class="text-caption font-weight-bold text-slate-400 text-uppercase mb-1">Ngày đăng ký</div>
                                    <div class="text-subtitle-2 font-weight-bold text-slate-700">{{ formatDate(order.khachHang?.ngayTao) }}</div>
                                </div>
                            </v-col>
                            <v-col cols="12" md="3">
                                <div class="pl-md-4 border-l-md">
                                    <div class="text-caption font-weight-bold text-slate-400 text-uppercase mb-1">Hạng khách hàng</div>
                                    <v-chip size="small" color="primary" variant="flat" class="font-weight-bold px-3 text-white">Thành viên</v-chip>
                                </div>
                            </v-col>
                        </v-row>
                    </v-card-text>
                ...
                <v-card class="premium-card mb-6 overflow-hidden">
                    <div class="card-title pa-5 border-b d-flex align-center justify-space-between bg-slate-50">
                        ...
                        <v-btn v-if="getOrderStatus() !== null && getOrderStatus() < 3" variant="flat" color="primary"
                            size="small" class="px-4 font-weight-bold rounded-md text-white" @click="productsDialogOpen = true">
                            <PlusIcon size="16" class="mr-2 text-white" />
                            Thêm / sửa sản phẩm
                        </v-btn>
                    </div>
                    <div class="table-scroll-container" style="max-height: 480px; overflow-y: auto;">
                        <v-table class="premium-table" density="compact">
                            <thead>
                                <tr>
                                    <th class="text-center py-4 font-weight-bold">STT</th>
                                    <th class="text-center py-4 font-weight-bold">Tên sản phẩm</th>
                                    <th class="text-center py-4 font-weight-bold">Mã biến thể</th>
                                    <th class="text-center py-4 font-weight-bold">Màu sắc</th>
                                    <th class="text-center py-4 font-weight-bold">Kích thước</th>
                                    <th class="text-center py-4 font-weight-bold">Số lượng</th>
                                    <th class="text-center py-4 font-weight-bold">Đơn giá</th>
                                    <th class="text-center py-4 font-weight-bold">Thành tiền</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(item, index) in order.listsHoaDonChiTiet" :key="item.id" class="hover-row">
                                    <td class="text-center text-slate-400 font-weight-medium">{{ index + 1 }}</td>
                                    <td class="py-4">
                                        <div class="d-flex align-center justify-center text-center">
                                            <div class="font-weight-bold text-slate-800">
                                                {{ item.tenSanPham || item.sanPhamChiTiet?.sanPham?.tenSanPham || item.chiTietSanPham?.sanPham?.ten || item.ten || 'N/A' }}
                                            </div>
                                        </div>
                                    </td>
                                    <td class="text-center font-weight-bold text-slate-600">
                                        {{ item.chiTietSanPham?.maChiTietSanPham || item.maChiTietSanPham || item.maBienThe || 'N/A' }}
                                    </td>
                                    <td class="text-center">
                                        <v-chip size="x-small" variant="tonal" color="primary" class="font-weight-bold">
                                            {{ item.tenMauSac || item.sanPhamChiTiet?.mauSac?.ten || item.chiTietSanPham?.mauSac?.ten || item.mauSac?.ten || 'N/A' }}
                                        </v-chip>
                                    </td>
                                    <td class="text-center">
                                        <v-chip size="x-small" variant="tonal" color="secondary" class="font-weight-bold">
                                            {{ item.tenKichThuoc || item.sanPhamChiTiet?.kichThuoc?.ten || item.chiTietSanPham?.kichThuoc?.ten || item.kichThuoc?.ten || 'N/A' }}
                                        </v-chip>
                                    </td>
                                    <td class="text-center font-weight-bold">
                                        {{ item.soLuong }}
                                    </td>
                                    <td class="text-right text-slate-700 font-weight-bold">{{ formatCurrency(item.donGia) }}</td>
                ...
                <v-card class="premium-card mb-6">
                    ...
                    <v-card-text class="pa-0">
                        <div class="history-scroll-container pa-6">
                            <v-timeline side="end" align="start" density="compact" class="full-width-timeline">
                                    ...
                                    <v-row no-gutters align="center" class="w-100">
                                        <v-col cols="12" md="2" class="pr-2">
                                            <div class="d-flex flex-column">
                                                <span class="text-caption text-slate-400 font-weight-bold mb-1">Trạng thái:</span>
                                                <span class="font-weight-bold text-slate-800 whitespace-nowrap">{{ getStatusInfo(log.trangThaiMoi).text }}</span>
                                            </div>
                                        </v-col>
                                        <v-col cols="12" md="4" class="pr-2">
                                            <div class="d-flex flex-column">
                                                <span class="text-caption text-slate-400 font-weight-bold mb-1">Mô tả:</span>
                                                <span class="text-body-2 text-slate-600 italic">{{ log.ghiChu || 'Hệ thống cập nhật' }}</span>
                                            </div>
                                        </v-col>
                                        <v-col cols="12" md="3" class="text-md-center py-2 py-md-0">
                                            <div class="d-inline-flex align-center">
                                                <span class="text-caption text-slate-400 font-weight-bold mr-2">Người thực hiện:</span>
                                                <v-chip size="x-small" color="primary" variant="tonal" class="font-weight-bold px-3">
                                                    <v-icon start size="12">mdi-account-circle-outline</v-icon>
                                                    {{ log.nguoiThucHien || 'ADMIN' }}
                                                </v-chip>
                                            </div>
                                        </v-col>
                                        <v-col cols="12" md="3" class="text-md-right text-caption text-slate-400 font-weight-bold">
                                            <div class="d-inline-flex align-center">
                                                <span class="text-caption text-slate-400 font-weight-bold mr-2">Thời gian:</span>
                                                <span class="text-slate-500">{{ formatDate(log.ngayTao) }}</span>
                                            </div>
                                        </v-col>
                                    </v-row>
                                </v-timeline-item>
                                <v-timeline-item v-if="!order.listsLichSuHoaDon || order.listsLichSuHoaDon.length === 0"
                                    ...
                                    <v-row no-gutters align="center" class="w-100">
                                        <v-col cols="12" md="3" class="pr-2">
                                            <div class="d-flex flex-column">
                                                <span class="text-caption text-slate-400 font-weight-bold mb-1">Trạng thái:</span>
                                                <span class="font-weight-bold text-slate-800">Khởi tạo</span>
                                            </div>
                                        </v-col>
                                        <v-col cols="12" md="3" class="pr-2">
                                            <div class="d-flex flex-column">
                                                <span class="text-caption text-slate-400 font-weight-bold mb-1">Mô tả:</span>
                                                <span class="text-body-2 text-slate-600 italic">Đơn hàng được ghi nhận</span>
                                            </div>
                                        </v-col>
                                        <v-col cols="12" md="3" class="text-md-center py-2 py-md-0">
                                            <div class="d-inline-flex align-center">
                                                <span class="text-caption text-slate-400 font-weight-bold mr-2">Người thực hiện:</span>
                                                <v-chip size="x-small" color="success" variant="tonal" class="font-weight-bold px-3">
                                                    <v-icon start size="12">mdi-star-outline</v-icon>
                                                    HỆ THỐNG
                                                </v-chip>
                                            </div>
                                        </v-col>
                                        <v-col cols="12" md="3" class="text-md-right text-caption text-slate-400 font-weight-bold">
                                            <div class="d-inline-flex align-center">
                                                <span class="text-caption text-slate-400 font-weight-bold mr-2">Thời gian:</span>
                                                <span class="text-slate-500">{{ formatDate(order.ngayTao) }}</span>
                                            </div>
                                        </v-col>
                                    </v-row>
                                </v-timeline-item>
            ...
            <v-col cols="12" lg="4">
                ...
                <v-card class="premium-card mb-6">
                    <div class="card-title pa-5 border-b d-flex align-center justify-space-between bg-slate-50">
                        ...
                        <v-btn variant="flat" color="primary" size="small" class="rounded-md px-4 font-weight-bold text-white" @click="openEditShipping">
                            <EditIcon size="16" class="mr-2 text-white" />
                            Chỉnh sửa
                        </v-btn>
                    </div>
                ...
                <v-card class="premium-card">
                    ...
                    <v-card-text class="pa-6">
                        ...
                        <AdminEmptyState v-else 
                            ...
                            subtitle="Đơn hàng này hiện chưa ghi nhận bất kỳ giao dịch thanh toán nào."
                            class="py-8"
                            icon-size="32"
                        />
                    </v-card-text>
        ...
        <v-dialog v-model="receptionistDialogOpen" max-width="400" transition="dialog-bottom-transition">
            <v-card class="rounded-md">
                ...
                <v-card-actions class="pa-6 pt-0">
                    ...
                    <v-spacer></v-spacer>
                    <v-btn color="primary" variant="flat" class="rounded-md px-8 text-white" :loading="updatingInfo" @click="saveInfoUpdate('receptionist')">Cập nhật</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="customerDialogOpen" max-width="500" transition="dialog-bottom-transition">
            <v-card class="rounded-md">
                <v-card-title class="pa-5 border-b font-weight-bold">Chỉnh sửa thông tin khách hàng</v-card-title>
                <v-card-text class="pa-6">
                    <v-text-field v-model="editInfoForm.tenKhachHang" label="Tên khách hàng"
                        placeholder="Nhập tên khách hàng" variant="outlined" class="mb-4" hide-details></v-text-field>
                    <v-text-field v-model="editInfoForm.soDienThoai" label="Số điện thoại"
                        placeholder="Nhập số điện thoại" variant="outlined" class="mb-4" hide-details></v-text-field>
                    <v-text-field v-model="editInfoForm.email" label="Email"
                        placeholder="Nhập email" variant="outlined" hide-details></v-text-field>
                </v-card-text>
                <v-card-actions class="pa-6 pt-0">
                    <v-btn variant="tonal" color="slate-500" class="rounded-md px-6" @click="customerDialogOpen = false">Hủy</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn color="primary" variant="flat" class="rounded-md px-8 text-white" :loading="updatingInfo" @click="saveInfoUpdate('customer')">Lưu thay đổi</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="shippingDialogOpen" max-width="600" transition="dialog-bottom-transition">
            <v-card class="rounded-md">
                <v-card-title class="pa-5 border-b font-weight-bold">Chỉnh sửa thông tin vận chuyển</v-card-title>
                <v-card-text class="pa-6">
                    <v-row>
                        <v-col cols="12" md="6">
                            <v-text-field v-model="editInfoForm.soDienThoaiNguoiNhan" label="Số điện thoại nhận hàng"
                                placeholder="Nhập số điện thoại" variant="outlined" hide-details></v-text-field>
                        </v-col>
                        <v-col cols="12" md="6">
                            <v-autocomplete v-model="editInfoForm.province" :items="provinces" item-title="name"
                                item-value="code" label="Tỉnh/Thành phố" variant="outlined" hide-details
                                :loading="loadingLoc.provinces" @update:model-value="fetchDistricts"></v-autocomplete>
                        </v-col>
                        <v-col cols="12" md="6">
                            <v-autocomplete v-model="editInfoForm.district" :items="districts" item-title="name"
                                item-value="code" label="Quận/Huyện" variant="outlined" hide-details
                                :loading="loadingLoc.districts" @update:model-value="fetchWards"
                                :disabled="!editInfoForm.province"></v-autocomplete>
                        </v-col>
                        <v-col cols="12" md="6">
                            <v-autocomplete v-model="editInfoForm.ward" :items="wards" item-title="name"
                                item-value="code" label="Phường/Xã" variant="outlined" hide-details
                                :loading="loadingLoc.wards" :disabled="!editInfoForm.district"></v-autocomplete>
                        </v-col>
                        <v-col cols="12">
                            <v-textarea v-model="editInfoForm.detailAddress" label="Địa chỉ chi tiết"
                                placeholder="Số nhà, tên đường..." variant="outlined" rows="2" hide-details></v-textarea>
                        </v-col>
                        <v-col cols="12">
                            <v-textarea v-model="editInfoForm.ghiChu" label="Ghi chú giao hàng"
                                placeholder="Nhập ghi chú cho đơn vị vận chuyển" variant="outlined" rows="2" hide-details></v-textarea>
                        </v-col>
                    </v-row>
                </v-card-text>
                <v-card-actions class="pa-6 pt-0">
                    ...
                    <v-spacer></v-spacer>
                    <v-btn color="primary" variant="flat" class="rounded-md px-8 text-white" :loading="updatingInfo" @click="saveInfoUpdate('shipping')">Lưu thay đổi</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
                                ...
                                <v-table class="items-table-compact">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center py-3 text-caption font-weight-bold">Mã</th>
                                            <th class="text-center py-3 text-caption font-weight-bold">Số lượng</th>
                                        ...
                                        <tr v-for="item in order.listsHoaDonChiTiet" :key="item.id" class="hover-row">
                                            <td class="py-3">
                                                ...
                                                <div class="text-caption text-slate-500">
                                                    {{ item.tenMauSac || item.chiTietSanPham?.mauSac?.ten || '—' }} | {{ item.tenKichThuoc || item.chiTietSanPham?.kichThuoc?.ten || '—' }}
                                                </div>
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex align-center justify-center bg-slate-50 rounded-lg pa-1" style="min-width: 80px;">
                                        ...
                                        <tr v-if="!order.listsHoaDonChiTiet?.length">
                                            <td colspan="3" class="pa-0">
                                                <AdminEmptyState 
                                                    ...
                                                    title="Trống" 
                                                    subtitle="Vui lòng chọn sản phẩm bên trái."
                                                    class="py-10"
                                                    icon-size="32"
                                                />
                                            </td>
...
.full-width-timeline :deep(.v-timeline-item__body) {
