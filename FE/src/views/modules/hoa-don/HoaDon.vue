...
const counts = ref({
    all: 0,
    pendingPayment: 0,  // 0: CHO_THANH_TOAN
    processing: 0,      // 1: CHO_XAC_NHAN
    shipped: 0,         // 2: CHO_GIAO_HANG
    delivered: 0,       // 3: DANG_VAN_CHUYEN
    daGiaoHang: 0,      // 4: DA_GIAO_HANG
    refunded: 0,        // 5: DA_THANH_TOAN
    completed: 0,       // 6: HOAN_THANH
    daHuy: 0,           // 7: DA_HUY
    cancelRequest: 0,   // 8: YEU_CAU_HUY
    refundNeeded: 0     // 9: CAN_HOAN_PHI
});
...
const loadCounts = async () => {
    try {
        ...
        counts.value = {
            all: data.all || 0,
            // BE uses OrderStatus ordinal (EnumType.ORDINAL): 0..9
            pendingPayment: data['0'] || 0,   // CHO_THANH_TOAN
            processing: data['1'] || 0,       // CHO_XAC_NHAN
            shipped: data['2'] || 0,          // CHO_GIAO_HANG
            delivered: data['3'] || 0,        // DANG_VAN_CHUYEN
            daGiaoHang: data['4'] || 0,       // DA_GIAO_HANG
            refunded: data['5'] || 0,         // DA_THANH_TOAN
            completed: data['6'] || 0,        // HOAN_THANH
            daHuy: data['7'] || 0,            // DA_HUY
            cancelRequest: data['8'] || 0,    // YEU_CAU_HUY
            refundNeeded: data['9'] || 0      // CAN_HOAN_PHI
        };
    } catch (e) {
            ...
            <template #top>
                ...
                >
                    ...
                    <v-tab :value="4" class="text-none font-weight-bold px-2 tab-item">
                        ...
                        Đã giao hàng
                        <v-avatar
                            v-if="hasCount(counts.daGiaoHang)"
                            size="24"
                            color="#6366f1"
                            class="ml-2 tab-count-avatar tab-count-status font-weight-black"
                        >
                    ...
                    <v-tab :value="7" class="text-none font-weight-bold px-2 tab-item">
                        <v-icon start size="16">mdi-close-circle-outline</v-icon>
                        Đã hủy
                        <v-avatar
                            v-if="hasCount(counts.daHuy)"
                            size="24"
                            color="#ef4444"
                            class="ml-2 tab-count-avatar tab-count-status font-weight-black"
                        >
                    ...
                    <v-tab :value="8" class="text-none font-weight-bold px-2 tab-item">
                        <v-icon start size="16">mdi-alert-circle-outline</v-icon>
                        Yêu cầu hủy
                        <v-avatar
                            v-if="hasCount(counts.cancelRequest)"
                            size="24"
                            color="#f97316"
                            class="ml-2 tab-count-avatar tab-count-status font-weight-black"
                        >
                    ...
                    <v-tab :value="9" class="text-none font-weight-bold px-2 tab-item">
                        <v-icon start size="16">mdi-cash-return</v-icon>
                        Cần hoàn phí
                        <v-avatar
                            v-if="hasCount(counts.refundNeeded)"
                            size="24"
                            color="#7c3aed"
                            class="ml-2 tab-count-avatar tab-count-status font-weight-black"
                        >
                ...
                <tr class="data-row">
                    ...
                    <td class="data-cell status-cell">
                        <template v-if="getStatusMeta(item.trangThai)">
                            <v-chip
                                :class="['font-weight-black status-chip text-white', getStatusMeta(item.trangThai).chipClass]"
                                variant="flat"
                                size="small"
                            >
