...
const filteredCustomers = computed(() => {
    return customers.value.filter((c) => {
        ...
        return !search || c.ten?.toLowerCase().includes(search) || c.sdt?.includes(search) || c.maKh?.toLowerCase().includes(search) || c.diaChiChiTiet?.toLowerCase().includes(search);
    });
});
...
<template>
    <v-container fluid class="pa-6 animate-fade-in overflow-y-auto" style="height: 100vh;">
        ...
        <div class="d-flex align-center justify-space-between mb-8 mt-4">
            ...
            <div class="d-flex gap-3">
                <v-btn v-if="!isViewOnly" color="primary" variant="flat"
                    ...
                    {{ isEditMode ? 'Cập nhật thay đổi' : 'Thêm phiếu giảm giá' }}
                </v-btn>
            </div>
        </div>
        ...
        <!-- Layout 1: Public Voucher (CONG_KHAI) - Old Split Setup -->
        <v-row v-else-if="form.loaiHienThi === 'CONG_KHAI'" class="form-grid pb-16">
            <v-col cols="12" lg="8">
                    ...
                    <v-card-text class="pa-8">
                        ...
                        <v-row>
                            <v-col cols="12" md="4">
                                ...
                                <v-text-field v-model="form.ma" readonly variant="outlined" density="compact" class="bg-slate-50" hide-details></v-text-field>
                            </v-col>
                            <v-col cols="12" md="8">
                                <div class="field-label">Tên chương trình *</div>
                                <v-text-field v-model="form.ten" :readonly="isViewOnly" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <div class="field-label">Mô tả chi tiết</div>
                                <v-textarea v-model="form.moTa" :readonly="isViewOnly" variant="outlined" rows="3" hide-details></v-textarea>
                            </v-col>
                        </v-row>
                    </v-card-text>
                </v-card>
                    ...
                    <v-card-text class="pa-8">
                        ...
                        <v-row>
                            <v-col cols="12" md="4">
                                ...
                                <v-btn-toggle v-model="form.loaiPhieu" mandatory color="primary" variant="outlined" density="compact" class="w-100 rounded-lg">
                            ...
                            <v-col cols="12" md="4">
                                ...
                                <v-text-field :model-value="form.loaiPhieu === 'TIEN_MAT' ? form.soTienGiam : form.phanTramGiamGia" 
                                    @update:model-value="val => form.loaiPhieu === 'TIEN_MAT' ? (form.soTienGiam = Number(val)) : (form.phanTramGiamGia = Number(val))"
                                    :readonly="isViewOnly" type="number" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                            <v-col cols="12" md="4">
                                <div class="field-label">Giảm tối đa</div>
                                <v-text-field v-model.number="form.giamToiDa" :disabled="form.loaiPhieu === 'TIEN_MAT'" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                <div class="d-flex align-center gap-2 mb-2">
                                    <div class="field-label mb-0">Số lượng</div>
                                    <v-switch v-model="isInfinite" label="Vô hạn" color="primary" density="compact" hide-details class="custom-mini-switch"></v-switch>
                                </div>
                                <v-text-field v-model.number="form.soLuong" :disabled="isInfinite" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6">
                                <div class="field-label">Đơn tối thiểu</div>
                                <v-text-field v-model.number="form.giatriToiThieu" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                        </v-row>
                    </v-card-text>
                </v-card>
                    ...
                    <v-card-text class="pa-8">
                        ...
                        <v-row>
                            <v-col cols="12">
                                ...
                                <v-btn-toggle v-model="form.loaiHienThi" mandatory color="primary" variant="outlined" density="compact" class="w-100 rounded-lg">
                            ...
                            <v-col cols="12">
                                ...
                                <v-text-field v-model="form.ngayBatDau" type="datetime-local" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <div class="field-label">Kết thúc vào ngày</div>
                                <v-text-field v-model="form.ngayKetThuc" type="datetime-local" variant="outlined" density="compact" hide-details></v-text-field>
                            </v-col>
                        </v-row>
                    </v-card-text>
                </v-card>
        ...
        <!-- Layout 2: Individual Voucher (CA_NHAN) - New Row Layout -->
        <template v-else-if="form.loaiHienThi === 'CA_NHAN'">
            <v-row class="form-grid pb-6">
                <!-- 1. General Identification -->
                <v-col cols="12" md="4">
                    <v-card class="filter-card elevation-0 h-100 mb-0">
                            ...
                            <v-row>
                                <v-col cols="12">
                                    ...
                                    <v-text-field v-model="form.ma" readonly placeholder="Hệ thống tự tạo..."
                                        variant="outlined" density="compact" class="custom-input mono-font bg-slate-50"
                                ...
                                <v-col cols="12">
                                    ...
                                    <v-textarea v-model="form.moTa" :readonly="isViewOnly" variant="outlined" rows="3"
                                        placeholder="Điều kiện chương trình..." hide-details
                    ...
                    <v-card class="filter-card elevation-0 h-100 mb-0">
                        <v-card-text class="pa-6">
                            ...
                            <v-row>
                                <v-col cols="12">
                                    <div class="field-label">Hình thức giảm</div>
                                    <v-btn-toggle v-model="form.loaiPhieu" :disabled="isViewOnly" mandatory color="primary"
                                        variant="outlined" density="compact" class="w-100 rounded-lg custom-toggle">
                                        <v-btn value="TIEN_MAT" class="flex-grow-1">VNĐ</v-btn>
                                        <v-btn value="PHAN_TRAM" class="flex-grow-1">%</v-btn>
                                    </v-btn-toggle>
                                </v-col>
                                <v-col cols="12" md="6">
                                    ...
                                    <v-text-field v-if="form.loaiPhieu === 'TIEN_MAT'" v-model.number="form.soTienGiam"
                                        :readonly="isViewOnly" type="number" suffix="VNĐ" variant="outlined"
                                ...
                                <v-col cols="12">
                                    <div class="d-flex align-center gap-4 mb-2" style="height: 24px;">
                                        <div class="field-label mb-0">Số lượng</div>
                                        <v-switch v-model="isInfinite" :disabled="isViewOnly" label="Vô hạn" color="primary"
                                            density="compact" hide-details class="custom-mini-switch ml-2"></v-switch>
                                    </div>
                                    <v-text-field :model-value="isInfinite ? 'Vô hạn' : form.soLuong"
                                        ...
                                        :disabled="isInfinite || form.loaiHienThi === 'CA_NHAN' || isViewOnly"
                                        variant="outlined" density="compact" hide-details></v-text-field>
                                </v-col>
                                <v-col cols="12">
                                    <div class="field-label mb-2">Đơn tối thiểu (VNĐ)</div>
            ...
            <!-- Customer Selection Table (Full Width) -->
            <v-card class="filter-card elevation-0 mb-6">
                <v-card-text class="pa-8">
                    <div class="section-header d-flex align-center mb-6">
                        <div class="icon-blob bg-emerald-lighten-5 mr-3">
                    ...
                    <!-- Import AdminFilter for searching -->
                    <AdminFilter title="Tìm kiếm khách hàng" @refresh="searchGlobal = ''">
                        <v-col cols="12" md="4">
                            <div class="field-label">Từ khóa tìm kiếm</div>
                            <v-text-field v-model="searchGlobal" prepend-inner-icon="mdi-magnify"
                                placeholder="Tìm tên, SĐT, mã KH..." variant="outlined" density="compact" hide-details></v-text-field>
                        </v-col>
                    </AdminFilter>

                    <div class="table-wrapper border rounded-lg overflow-x-auto mt-4">
                        <table class="native-admin-table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="header-cell text-center">Tên phiếu</th>
                                    <th class="header-cell text-center">Mã phiếu</th>
                                    ...
                                    <th class="header-cell text-center">Địa chỉ</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(item, index) in paginatedCustomers" :key="item.id"
                                    ...
                                    <td class="data-cell text-center text-body-2 text-slate-500" style="max-width: 200px;">
                                        <div class="text-truncate">{{ item.diaChiChiTiet || 'N/A' }}</div>
                                        <v-tooltip v-if="item.diaChiChiTiet" activator="parent" location="top">{{ item.diaChiChiTiet }}</v-tooltip>
                                    </td>
                                </tr>
                                <tr v-if="paginatedCustomers.length === 0">
                                    <td colspan="9" class="pa-0">
                                        <AdminEmptyState 
                                            ...
                                            subtitle="Vui lòng kiểm tra lại từ khóa tìm kiếm."
                                            class="py-12"
                                            icon-size="32"
                                        />
                                    </td>
