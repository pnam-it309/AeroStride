...
<template>
  <div class="flex items-center justify-between px-4 py-3 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl transition-all duration-300">
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="flex items-center gap-1">
        <template v-for="(p, index) in pages" :key="index">
          <span 
            v-if="p === '...'" 
            class="w-8 text-center text-gray-400 font-bold"
          >
            {{ p }}
          </span>
          <button
